...
import { X, Briefcase, Users, MapPin, Clock, DollarSign, Trophy, CheckCircle } from 'lucide-react';

const DetailModal = ({ isOpen, formation, onClose, compatibilityScore, selectedCity }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (previousHistoryStateRef.current) {
  ...
  const busAvailability = selectedCity && formation.ville === selectedCity
    ? { available: true, label: 'Bus disponible dans votre ville' }
    : { available: false, label: 'Bus non renseigné pour cette ville' };
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Modal */}
          <div className="fixed inset-0 z-50 overflow-y-auto p-2 sm:p-4 mt-10">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative mx-auto flex min-h-full w-full max-w-3xl items-center justify-center py-2 sm:py-4"
            >
              <div className="w-full max-h-[90vh] overflow-hidden rounded-3xl bg-white shadow-2xl">
                {/* Header */}
                  ...
                  <div className="mx-auto max-w-2xl space-y-6">
                    ...
                    <div className="rounded-2xl border border-[#DCD5C6] bg-[#FFFEF9] p-4">
                      <h3 className="font-bold text-[#0E1A2B] flex items-center gap-2">
                        <MapPin size={18} className="text-[#B98A3B]" />
                        Transport & accès
                      </h3>
                      <p className={`mt-2 text-sm font-semibold ${busAvailability.available ? 'text-[#1F7A54]' : 'text-[#5C6A7D]'}`}>
                        {busAvailability.label}
                      </p>
                    </div>
                    ...
                    {/* Conditions d'accès */}
                    <div className="rounded-2xl border border-[#E8E3D6] bg-[#FAFAFA] p-4 pb-10">
